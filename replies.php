...
$success="";
	...
	$success="posted";
					...
					if($_FILES['file']['tmp_name'][0]==""){$count_images=0;}
	...
	if ((($_FILES["file"]["type"][$i] == "image/gif") || 
		...
		if ($_FILES["file"]["error"][$i] > 0)	{
			$successMessage= "Return Code: " . $_FILES["file"]["error"][$i] . "<br>";
		}
		else	{
			$successMessage= "Upload: " . $_FILES["file"]["name"][$i] . "<br>";
  
			if (file_exists("avatars/" . $_FILES["file"]["name"][$i]))	{
				$successMessage= $_FILES["file"]["name"][$i] . " already exists. ";
			}
			else	{ 
				$id2=uniqid();
				move_uploaded_file($_FILES["file"]["tmp_name"][$i],"avatars/" . $id2 . $_FILES["file"]["name"][$i]);
				$img_array.="avatars/".$id2 .$_FILES['file']['name'][$i].'|';
					...
					if ($num_rows5 == 0)	{
						$successMessage= "Sorry, this category is no longer available. You are being redirected to the main category page.";
						header("Refresh:3; URL=categories.php");
					}
					else if ($row6['user_suspend'] == 1)	{
						$successMessage= "You are currently suspended and are unable to post messages.";
					}
					else	{
						...
						if (!$results4)	{
							$successMessage= "Your message has not been saved.  Please try again later.";
						}
						else	{
...
$pageno=($page-1)*$perpage;
	...
	$sql1 = "SELECT post_id, post_content, post_by, post_date, post_edit, post_editby, post_edited, post_deleted, user_id, user_dp, avatar_path, user_name, user_level, role_name
		...
		<div class='col-lg-12'>
			<div class='panel panel-warning'>
				<div class='panel-heading'>
					<div class='row'>
						<div class='col-lg-1'>
							<img src="<?php echo $path1; ?>" alt="Image cannot be displayed." style="max-width: 100%" class='img-thumbnail'/>
						</div>
						<div class='col-lg-3'>
							<h4><a style="color:#c09853" href='viewuser.php?id=<?php echo $row1['user_id']; ?>'><?php echo $row1['user_name']; ?></a>
							<?php echo " (" . $row1['role_name'] . ")"; ?></h4>
							&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $user_level;?>
						</div>
						<div class='col-lg-2'>
							<h4>Created on:</h4>
...
<?php
	if ($frozen == 0)	{	//Display reply option
		...
		$sql12 = "SELECT user_id, COUNT(post_id) as num_posts
							FROM proj4_users LEFT JOIN proj4_posts ON user_id=post_by
							WHERE user_id=" .$_SESSION['user_id'].
							" GROUP BY post_by";
							
		$results12 = $con->query($sql12);
		$row12 = $results12->fetch_assoc();
	
		if ($row12['num_posts'] >= 10 && $row12['num_posts'] < 20)	{
		$userlevel="intermediate";
		}
		else if ($row12['num_posts'] >= 20)	{
		$userlevel="master";
		}else{
		$userlevel="newbie";	
		}
			$resultslevel = $con->query("SELECT * from proj4_images where user_level='".$userlevel."'");
		$userlevelres=$resultslevel->fetch_assoc();
		$user_level1 = $userlevelres['images_count'];
		//echo $user_level1;
