...
?>
	...
	<div class='row'>
		<ul class='nav nav-tabs'>
			<li class='active'><a href='userprofile.php'>Threads</a></li>
			<li><a href='userprofile2.php'>Posts</a></li>
		</ul>
	</div>
	<div class='row'>
		<br>
...
<?php
		$sql4 = "SELECT distinct cat_id, cat_name
				 FROM proj4_categories, proj4_topics
				 WHERE cat_id = topic_cat AND topic_by = " . $_SESSION['user_id'];
...
?>		
	<div class="panel panel-warning">
		<div class="panel-heading">
			<div class="row">
				<div class='col-lg-11'>
					<h3 class="panel-title"><?php echo $row4['cat_name'];?></h3>
				</div>
				<div class='col-lg-1'>
...
<?php
			$sql5 = "SELECT *
					 FROM proj4_topics
					 WHERE topic_cat = " . $row4['cat_id'] . " AND topic_by = " . $_SESSION['user_id'] .
					 " ORDER BY topic_date DESC";
			$results5 = $con->query($sql5);
			while ($row5 = $results5->fetch_assoc())	{
				$sql6 = "SELECT count(post_id) num_posts
					 FROM proj4_posts
					 WHERE post_topic = " . $row5['topic_id'] . " AND post_deleted = 0";
				$results6 = $con->query($sql6);
				$row6 = $results6->fetch_assoc();
?>
			<div class='row'>
				<div class='col-lg-11'>
					<a href='replies.php?id=<?php echo $row5['topic_id']?>'><?php echo $row5['topic_subject'];?></a>
				</div>
				<div class='col-lg-1'>
					<span class='badge'><?php echo $row6['num_posts'];?></span>
				</div>
			</div>
			<div class='row'>
				<div class='col-lg-10'>
					<p class='text-muted'><small><?php echo $row5['topic_date'];?></small></p>
				</div>
			</div>
